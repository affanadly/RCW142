...
from tasks import fitld, msort, indxr, uvflg, clcal, accor, snedt, antab, apcal, bpass, fring, tabed_key, cvel_doppler, set_velocity, split, fittp, calib, tacop
...
if __name__ == '__main__':
    ...
    selfcal = ps.add_argument_group('self-calibration', description='Self-calibration flag')
    selfcal.add_argument('--selfcal', action='store_false', help='Enable self-calibration')
    ...
    # self-calibration
    if args.selfcal:
        peak = split(uvdata, args.target, params={
            'flagver': 1 if args.flag_file is not None else -1,
            'bchan': 476,
            'echan': 476,
            'docalib': 1,
        ...
        fittp(peak, f'{os.path.split(args.file)[0]}/peak.fits')
        
        print('Perform self-calibration using Difmap and save the output file as "selfcal.fits".')
        input('Press enter to continue once completed...')
        
        selfcal = fitld(f'{os.path.split(args.file)[0]}/selfcal.fits', args.disk, args.clint, [args.target])
        calib(peak, selfcal, [args.target], params={
            'cmethod': 'DFT',
            'cmodel': 'COMP',
            'doapply': -1,
            'refant': args.refant,
        ...
        snedt(peak, invers=1, params={'dodelay': 1, 'solint': args.clint})
        tacop(peak, uvdata, 'SN', invers=2, ncount=1)
        clcal(uvdata, sources=[args.target], calsour=[args.target], params={
            'opcode': 'CALI',
            'interpol': 'SIMP',
            'smotype': 'VLBI',
    ...
    uvdata_final = split(uvdata, args.target, params={
        'flagver': 1 if args.flag_file is not None else -1,
        'docalib': 1,
        'gainuse': 6 if args.selfcal else 5,
        'doband': 2,
        'bpver': 1
    })
